...
export default function AddTopic() {
  ...
  const handleNext = () => {
    navigate(`/courses/${courseId}/topics/new/materials`, {
      state: { name: name.trim(), subtitle: subtitle.trim(), totalPages: Number(totalPages) || 10 }
    })
  }
    ...
    <div>
      ...
      <div className="max-w-2xl bg-surface-container-lowest rounded-xl p-stack-lg border border-outline-variant/30">
        <div className="space-y-stack-md">
          ...
          <div>
            <label className="font-label-caps text-label-caps text-primary-container block mb-unit">Subtitle</label>
            <input className="w-full bg-surface-container-low border-none rounded-xl px-4 py-3 font-body-md text-body-md focus:ring-2 focus:ring-primary" placeholder="e.g., Core Principle: Negligence" value={subtitle} onChange={e => setSubtitle(e.target.value)} />
          </div>

          <div>
            <label className="font-label-caps text-label-caps text-primary-container block mb-unit">Total Pages</label>
            <input type="number" min="1" className="w-full bg-surface-container-low border-none rounded-xl px-4 py-3 font-body-md text-body-md focus:ring-2 focus:ring-primary" placeholder="e.g., 10" value={totalPages} onChange={e => setTotalPages(e.target.value)} />
          </div>
