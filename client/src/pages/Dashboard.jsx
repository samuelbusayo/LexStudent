...
import { useCountdown, useBadges } from '../hooks/useBadges';
...
export default function Dashboard() {
  ...
  const { data: countdown, isLoading: countdownLoading } = useCountdown();
  const { data: badges, isLoading: badgesLoading } = useBadges();

  const isLoading = coursesLoading || overallLoading || recentLoading || countdownLoading || badgesLoading;
      ...
      <div className="col-span-12 lg:col-span-3 space-y-stack-md">
        ...
        <section className="bg-primary text-on-primary p-stack-lg rounded-xl shadow-lg relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-10">
            <span className="material-symbols-outlined text-[120px]" style={{ fontVariationSettings: "'FILL' 1" }}>gavel</span>
          </div>
          <h4 className="font-label-caps text-label-caps text-secondary-fixed mb-unit">UPCOMING MILESTONE</h4>
          <h3 className="font-h2 text-h2 mb-stack-md">{countdown?.title || 'Bar Finals'}</h3>
          <div className="space-y-stack-sm">
            <div className="flex justify-between items-end">
              <span className="text-5xl font-h1">{countdown?.daysRemaining ?? 42}</span>
              <span className="font-body-md opacity-80 pb-1">Days Remaining</span>
            </div>
          ...
          <p className="mt-stack-lg font-body-md text-on-primary/80 leading-relaxed">Intensive revision phase starts in 4 days. Ensure all Civil Litigation modules are completed.</p>
          <button className="mt-stack-lg w-full py-3 bg-secondary-container text-on-secondary-container rounded font-button hover:opacity-90 transition-all">View Study Plan</button>
        </section>
