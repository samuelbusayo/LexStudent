...
export default function RevisionSession() {
  ...
  const gaps = ((allGaps as any[]) || []).slice(0, 4)

  const [currentIndex, setCurrentIndex] = useState(0)
  const [results, setResults] = useState<{ topic: string; rating: string }[]>([])
  const [isRating, setIsRating] = useState(false)

  const reviewMutation = useMutation({
    mutationFn: ({ topicId, rating }: { topicId: number; rating: string }) =>
  ...
  if (gaps.length === 0) {
    return (
      <div className="px-5 py-12 max-w-2xl mx-auto text-center space-y-4">
        <span className="material-symbols-outlined text-secondary text-[64px]">celebration</span>
        <h1 className="font-h1 text-primary-container">All Caught Up!</h1>
        <p className="text-on-surface-variant font-body-lg">No topics need review right now. Great work!</p>
        <button onClick={() => navigate('/revision')} className="bg-primary-container text-white px-6 py-3 rounded-xl font-button">
          Back to Revision
        </button>
      </div>
    )
  ...
  if (isComplete) {
    return (
      <div className="px-5 py-12 max-w-2xl mx-auto space-y-6">
        <div className="text-center space-y-2">
          <span className="material-symbols-outlined text-secondary text-[64px]">emoji_events</span>
          <h1 className="font-h1 text-primary-container">Session Complete!</h1>
        ...
        <div className="text-center">
          <button onClick={() => navigate('/revision')} className="bg-primary-container text-white px-8 py-3 rounded-xl font-button">
            Back to Revision
          </button>
        </div>
      </div>
  ...
  return (
    <div className="px-5 py-8 max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl border border-outline-variant/30 p-8 space-y-4">
        ...
        <h2 className="font-h1 text-primary-container">{topic.topic}</h2>
        ...
        <p className="font-body-lg text-on-surface-variant pt-4 border-t border-outline-variant/30">
      ...
      <div className="grid grid-cols-3 gap-3">
        {RATINGS.map((r) => (
          ...
          >
            ...
            <span className="font-button">{r.label}</span>
          </button>
        ))}
      </div>
    </div>
