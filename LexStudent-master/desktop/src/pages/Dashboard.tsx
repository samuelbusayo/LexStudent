    ...
    <div className="grid grid-cols-12 gap-gutter">
        ...
        <section className="bg-primary text-on-primary p-stack-lg rounded-xl shadow-lg relative overflow-hidden">
          ...
          {milestone ? (
            ...
            <>
              ...
              <p className="font-body-md text-on-primary/80 leading-relaxed">Create a milestone to track your countdown.</p>
      ...
      <div className="col-span-12 lg:col-span-6 space-y-stack-md">
        <div className="flex items-center justify-between">
          ...
          <Link to="/planner" className="text-primary font-button flex items-center gap-1 hover:underline">
            Explore All <span className="material-symbols-outlined text-sm">arrow_forward</span>
          </Link>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-stack-md">
          {coursesArr.map((course: any, index: number) => {
              ...
              <Link to={`/courses/${course.id}`} key={course.id || index} className={`block ${isLastOdd ? 'col-span-1 md:col-span-2' : ''} bg-surface-container-lowest p-stack-md rounded-xl border border-outline-variant/30 hover:shadow-md transition-shadow`}>
                ...
                <div className="space-y-1">
                  ...
                  <div className="w-full bg-surface-container h-1 rounded-full overflow-hidden">
                    <div className="bg-secondary h-full transition-all duration-1000" style={{ width: `${course.progressPercent ?? 0}%` }} />
                  </div>
                </div>
              </Link>
            )
      ...
      <div className="col-span-12 lg:col-span-3 space-y-stack-lg">
            ...
            {(overallProgress?.courses || []).filter((c: any) => c.totalTopics > 0).slice(0, 4).map((c: any) => (
              <div key={c.courseId} className="space-y-2">
                ...
                <div className="w-full bg-surface-container h-1.5 rounded-full overflow-hidden">
                  <div className="bg-secondary h-full transition-all duration-1000" style={{ width: `${c.progressPercent}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="bg-surface-container-lowest p-stack-md rounded-xl border border-outline-variant/30">
          ...
          <div className="grid grid-cols-4 gap-2">
            {badgesArr.filter((b: any) => b.earned).slice(0, 4).map((badge: any) => (
              <div key={badge.id} className="aspect-square rounded-lg bg-surface-container-low flex items-center justify-center border border-outline-variant/10 cursor-help relative group">
                <span className="material-symbols-outlined text-secondary" style={{ fontVariationSettings: "'FILL' 1" }}>{badge.icon}</span>
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max px-2 py-1 bg-primary text-on-primary text-[10px] rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">{badge.name}</div>
              </div>
            ))}
            {Array.from({ length: Math.max(0, 4 - badgesArr.filter((b: any) => b.earned).length) }).map((_, i) => (
