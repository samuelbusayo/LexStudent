...
export function useMilestoneMutations() {
  ...
  const create = useMutation({
    mutationFn: (data: any) => api.post('/badges/milestone', data).then((r) => r.data),
    onSuccess: invalidate,
  })

  const update = useMutation({
    mutationFn: ({ id, ...data }: any) => api.put(`/badges/milestone/${id}`, data).then((r) => r.data),
    onSuccess: invalidate,
  })

  const remove = useMutation({
    mutationFn: (id: number) => api.delete(`/badges/milestone/${id}`).then((r) => r.data),
    onSuccess: invalidate,
  })
