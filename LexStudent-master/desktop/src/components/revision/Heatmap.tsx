...
export default function Heatmap() {
  ...
  const intensities: number[] = data?.intensities || Array(21).fill(0)
  const targetMet: number = data?.targetMet ?? 0
  const targetTotal: number = data?.targetTotal ?? 5

  return (
    <div className="bg-white rounded-xl p-6 border border-outline-variant/30 flex flex-col justify-between">
      <div>
        <h3 className="font-h3 text-primary-container mb-4">Revision Intensity</h3>
        <div className="grid grid-cols-7 gap-2">
          {intensities.map((v: number, i: number) => (
