...
export default function AiChatPanel({ topicId, onNavigateToPage }: { topicId: string | undefined; onNavigateToPage: (page: number) => void }) {
  ...
  const { isPremium, messagesUsed, messagesLimit } = useSubscription()
  ...
  if (!isExpanded) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-50 flex items-center gap-2 px-4 py-3 bg-primary-container text-white rounded-full shadow-lg hover:shadow-xl hover:opacity-95 transition-all active:scale-95"
      >
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 w-[400px] h-[520px] bg-surface-container-lowest rounded-2xl shadow-2xl border border-outline-variant/30 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-h3 text-sm text-primary-container">
              AI Study Assistant
              {isPremium && <span className="ml-1.5 text-[9px] px-1.5 py-0.5 bg-amber-100 text-amber-700 rounded-full font-bold align-middle">PRO</span>}
            </h3>
            <span className="text-[10px] text-on-surface-variant">
              {aiStatus?.indexStatus === 'completed' ? `${aiStatus.totalChunks} chunks indexed`
                ...
                : 'Ready'}
              {!isPremium && <span className="ml-1.5 text-on-surface-variant/60">({messagesUsed}/{messagesLimit} msgs)</span>}
            </span>
          </div>
        </div>
        <div className="flex items-center gap-1">
          {messages.length > 0 && (
            <button onClick={clearConversation} className="p-1.5 text-on-surface-variant hover:text-error rounded transition-colors" title="Clear conversation">
              <span className="material-symbols-outlined text-[18px]">delete_sweep</span>
            </button>
          )}
          <button onClick={() => setIsExpanded(false)} className="p-1.5 text-on-surface-variant hover:text-on-surface rounded transition-colors" title="Minimize">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {messages.map((msg) => (
          <ChatMessage key={msg.id} message={msg} onPageClick={onNavigateToPage} onEdit={editMessage} onRetry={retryMessage} isLoading={isLoading} />
        ))}
        {error && (
          <div className="text-xs text-error bg-error-container/20 rounded-lg px-3 py-2 border border-error/20">{error}</div>
        )}
      ...
      <div className="p-3 border-t border-outline-variant/30 flex-shrink-0">
        {isLimitReached ? (
          <a
            href="/subscription"
            className="block w-full py-2.5 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl text-sm font-bold hover:bg-amber-100 transition-colors text-center"
          >
            Upgrade to Premium
          </a>
        ) : (
