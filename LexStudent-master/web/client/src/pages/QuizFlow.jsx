...
// ── Scenario renderer with blank chips ──
function ScenarioPanel({ scenario, currentBlank, collapsed, onToggle }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E0E0D0] overflow-hidden">
      <button
        onClick={onToggle}
      ...
      <div className={`p-5 pt-0 md:p-5 ${collapsed ? 'hidden md:block' : ''}`}>
        ...
        <div className="font-body-md text-on-surface leading-relaxed whitespace-pre-line">
          {renderText(scenario.text)}
        </div>
      </div>
    </div>
  );
...
export default function QuizFlow() {
  ...
  // Flow state
  const [step, setStep] = useState(STEP_COURSE);
  ...
  const currentQuestion = sessionData?.questions?.find(q => q.id === currentQuestionId);
  const currentScenario = currentQuestion?.scenarioId
    ? sessionData?.scenarios?.find(s => s.id === currentQuestion.scenarioId)
    : null;

  // Auto-expand scenario when it changes
  useEffect(() => {
  ...
  const advanceQuestion = useCallback(async (optionIdx) => {
    ...
    try {
      ...
      await submitAnswer.mutateAsync({
        ...
        selectedIndex: optionIdx ?? null,
        timeTakenSeconds: secondsPerQ - timeLeft,
      });
  ...
  const handleStartQuiz = async () => {
    try {
      const data = await startAttempt.mutateAsync({
        ...
        questionOrder,
        numQuestions: numQuestions === 'all' ? 'all' : Number(numQuestions),
      });
  ...
  if (step === STEP_COURSE) {
    return (
      <div className="px-container-padding py-6 max-w-5xl mx-auto space-y-6">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/revision')} className="text-on-surface-variant hover:text-primary-container">
            <span className="material-symbols-outlined">arrow_back</span>
          </button>
          <div>
            <h1 className="font-h1 text-h1 text-primary-container">Choose a Course</h1>
            ...
            {(courses || []).map(course => {
              ...
              return (
                <button
                  ...
                  disabled={disabled}
                  className={`text-left bg-surface-container-lowest p-stack-md rounded-xl border transition-all ${
                    disabled
                      ? 'border-outline-variant/20 opacity-50 cursor-not-allowed'
                      : 'border-outline-variant/30 hover:shadow-md hover:border-secondary/50 cursor-pointer'
                  }`}
                >
                  <div className="flex justify-between items-start mb-2">
                    <span className="bg-on-primary-container/10 text-on-primary-container text-[10px] px-2 py-1 rounded font-bold">
                      {course.type || 'CORE'}
                    </span>
                    {!disabled && (
                  ...
                  <h3 className="font-h3 text-h3 text-primary mb-1">{course.name}</h3>
                  <p className="text-xs text-on-surface-variant">
                    {disabled
                      ? 'No quiz questions available yet'
                      : `${course.questionTopicCount} topic${course.questionTopicCount !== 1 ? 's' : ''} with questions`}
                  </p>
                </button>
              );
            })}
  ...
  if (step === STEP_TOPIC) {
    return (
      <div className="px-container-padding py-6 max-w-3xl mx-auto space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-h1 text-h1 text-primary-container">Choose a Topic</h1>
              ...
              return (
                <button
                  ...
                  className={`w-full text-left flex items-center justify-between p-4 rounded-xl border transition-all ${
                    hasQuestions
                      ? 'bg-surface-container-lowest border-outline-variant/30 hover:shadow-md hover:border-secondary/50'
                      : 'bg-surface-container-lowest border-outline-variant/20 opacity-50 cursor-not-allowed'
                  }`}
                >
                  <div className="flex-1">
                    <h3 className="font-body-md font-bold text-primary-container">{topic.name}</h3>
                    <p className="text-xs text-on-surface-variant mt-0.5">
                      {hasQuestions ? `${topic.questionCount} question${topic.questionCount !== 1 ? 's' : ''}` : 'Questions coming soon'}
                    </p>
                  </div>
                  {hasQuestions ? (
  ...
  if (step === STEP_OPTIONS) {
    const totalAvailable = sessionData?.questions?.length || 0;
    return (
      <div className="px-container-padding py-6 max-w-2xl mx-auto space-y-6">
        <div className="flex items-center gap-3">
          <button onClick={() => { setStep(STEP_TOPIC); setSelectedTopicId(null); }} className="text-on-surface-variant hover:text-primary-container">
            <span className="material-symbols-outlined">arrow_back</span>
          </button>
          <div>
            <h1 className="font-h1 text-h1 text-primary-container">Quiz Options</h1>
            <p className="font-body-md text-on-surface-variant">{selectedTopicName}</p>
          </div>
        </div>

        <div className="bg-white rounded-xl border border-[#E0E0D0] p-6 space-y-6">
          {/* Time per question */}
          <div>
            ...
            <div className="flex gap-2">
              {[10, 15, 20, 30].map(sec => (
                <button
                  key={sec}
                  onClick={() => setSecondsPerQ(sec)}
                  className={`px-4 py-2 rounded-lg text-sm font-button transition-all ${
                    secondsPerQ === sec
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[
                ...
                <button
                  key={opt.value}
                  onClick={() => setQuestionOrder(opt.value)}
                  className={`px-4 py-2 rounded-lg text-sm font-button transition-all ${
                    questionOrder === opt.value
                      ? 'bg-secondary text-primary-container'
                      : 'bg-surface-container text-on-surface-variant hover:bg-surface-container-high'
                  }`}
                >
        ...
        <button
          ...
          className="w-full bg-primary-container text-white py-4 rounded-xl font-button text-button hover:brightness-110 transition-all disabled:opacity-50"
        >
  ...
  if (step === STEP_QUIZ && currentQuestion) {
    ...
    return (
      <div className="px-container-padding py-6 max-w-5xl mx-auto space-y-4">
        {/* Top bar */}
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <span className="font-label-caps text-on-surface-variant">
              Question {currentQIndex + 1} of {questionIds.length}
            </span>
        ...
        <div className={`grid gap-4 ${currentScenario ? 'md:grid-cols-2' : 'md:grid-cols-1 max-w-3xl mx-auto'}`}>
          ...
          {currentScenario && (
            <div className="md:sticky md:top-4 md:self-start">
              <ScenarioPanel
                ...
                currentBlank={isBlank ? currentQuestion.blankNumber : null}
          ...
          {/* Question card */}
          <div className="bg-white rounded-xl border border-[#E0E0D0] p-6 space-y-4">
            <div className="flex items-center gap-2 text-xs text-on-surface-variant">
              <span className="bg-primary-container/5 text-primary-container px-2 py-0.5 rounded-full font-bold tracking-widest uppercase text-[10px]">
                {selectedCourseName}
              </span>
              <span>|</span>
                ...
                <button
                  ...
                  className={`w-full text-left flex items-center gap-3 p-3 rounded-lg border-2 transition-all ${
                    selectedOption === idx
                      ...
                      : 'border-outline-variant/30 hover:border-secondary/50 bg-surface-container-lowest'
                  }`}
                >
      ...
      <div className="px-container-padding py-6 max-w-3xl mx-auto space-y-6">
        ...
        <div className="text-center pb-8">
          <button
            ...
            className="bg-primary-container text-white px-8 py-3 rounded-xl font-button hover:brightness-110 transition-all"
          >
            Back to Revision
          </button>
        </div>
...
// ── Review card for results ──
function ReviewCard({ index, question, scenario, isBlank, optionLetters }) {
  const [showScenario, setShowScenario] = useState(false);

  return (
    <div className="bg-white rounded-xl border border-[#E0E0D0] p-5 space-y-3">
      <div className="flex items-center justify-between">
