...
export default function StreakHero({ streak, badgeName }) {
  const days = streak?.streak ?? 12;
  const badge = badgeName ?? streak?.badge ?? 'Juris Master';

  return (
    <section className="bg-white rounded-xl p-6 border border-[#E0E0D0] shadow-[0_4px_6px_-1px_rgba(0,33,71,0.08)] flex items-center justify-between">
      ...
      <div className="relative w-24 h-24 flex items-center justify-center">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 96 96">
          ...
          <circle cx="48" cy="48" r="40" fill="transparent" stroke="currentColor" strokeWidth="4" strokeDasharray="251.2" strokeDashoffset="50.2" className="text-secondary" />
        </svg>
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="font-h3 text-primary-container">80%</span>
        </div>
      </div>
    </section>
  );
